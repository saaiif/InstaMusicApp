...
export default class LandingPage extends Component {
	...
	nextSong = (e) => {
		if (this.state.repeatSongStatus) {
			...
			this.setState({
				currentSongPos: Math.floor(Math.random() * this.state.PlaylistData.length),
				SongPlayStatus: true
			});
		} else {
	...
	render() {
		const currentSongTrack =
			this.state.PlaylistData.length === 0 ? 0 : this.state.PlaylistData[this.state.currentSongPos];
		return (
			<div className={classes.Container}>
				<div>
					...
					/>
					<Playlist playlistData={this.state.PlaylistData} onCardClick={this.cardClick} />
				</div>
			</div>
		);
	}
